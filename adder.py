...
# TO DO: Remove code input(just to check if an account is banned)
...
print(f'{info}{lg} ᴛᴏᴛᴀʟ ᴀᴄᴄᴏᴜɴᴛꜱ: {w}{len(accounts)}')
number_of_accs = int(input(f'{INPUT}{cy} ᴇɴᴛᴇʀ ɴᴜᴍʙᴇʀ ᴏꜰ ᴀᴄᴄᴏᴜɴᴛꜱ ᴛᴏ ᴜꜱᴇ: {r}'))
print(f'{info}{cy} ᴄʜᴏᴏꜱᴇ ᴀɴ ᴏᴘᴛɪᴏɴ{lg}')
...
with open('vars.txt', 'wb') as f:
    ...
    for ab in to_use:
        pickle.dump(ab, f)
    f.close()
sleep_time = int(input(f'{INPUT}{cy} ᴇɴᴛᴇʀ ᴅᴇʟᴀʏ ᴛɪᴍᴇ ᴘᴇʀ ʀᴇQᴜᴇꜱᴛ  {w}[{lg}0 for None{w}]: {r}'))
#print(f'{info}{lg} ᴊᴏɪɴɪɴɢ ɢʀᴏᴜᴘ ꜰʀᴏᴍ {w}{number_of_accs} accounts...')
#print(f'{grey}-'*50)
print(f'{success}{lg} -- ᴀᴅᴅɪɴɢ ᴍᴇᴍʙᴇʀꜱ ꜰʀᴏᴍ {w}{len(to_use)}{lg} account(s) --')
adding_status = 0
